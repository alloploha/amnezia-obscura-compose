...
DEFAULT_CONTAINER_NAME="amnezia-awg"
DEFAULT_DATA_DIR="/srv/amnezia/awg"
DEFAULT_CONTAINER_PATH="/opt/amnezia/awg"
...
usage() {
    cat <<'EOF'
Usage: externalize-amnezia-awg.sh [options]

This one-shot script migrates an existing Amnezia AWG container to use a
...
The script:
  ...
  2. recreates the same container name with a bind mount on /opt/amnezia/awg
  3. preserves existing non-AWG bind mounts such as a mounted /opt/amnezia/start.sh
  4. reconnects the recreated container to its non-default Docker networks

Options:
  --container <name>         Container name to migrate
  --data-dir <path>          Host data directory
  --container-path <path>    Path inside the container, default /opt/amnezia/awg
  --force                    Allow reuse of a non-empty host directory
  --verbose                  More output
  -h, --help                 Show this help

Examples:
  sudo bash scripts/externalize-amnezia-awg.sh
  sudo bash scripts/externalize-amnezia-awg.sh \
    --container amnezia-awg \
    --data-dir /srv/amnezia/awg
EOF
}
...
copy_container_data() {
    ...
    for required in awg0.conf wireguard_server_private_key.key wireguard_server_public_key.key wireguard_psk.key; do
        if [[ ! -s "$DATA_DIR/$required" ]]; then
            err "Required AWG state file was not copied or is empty: $required"
            exit 1
        fi
    done
}
...
set_permissions() {
    ...
    find "$DATA_DIR" -type d -exec chmod 700 {} \; || true
    find "$DATA_DIR" -type f -exec chmod 600 {} \; || true
}
...
main() {
    ...
    log "Creating replacement container with host-mounted AWG state"
    ...
    log "Host AWG state dir: $DATA_DIR"
